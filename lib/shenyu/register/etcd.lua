...
function _M.init(conf)
  ...
  local ok, err = ngx_timer_at(0, watch, rivision)
  if not ok then
...
local function parse_base_url(base_url)
  ...
  local base_url = m[1] .. "://" .. m[2] .. ":" .. conf.port
  local conf = {
    scheme = m[1],
    host = m[2],
    port = tonumber(m[3]),
  ...
  etcd_conf.base_url = conf.sheme .. conf.host .. ":" .. conf.port
  etcd_conf.
  return etcd_conf
end
...
local function update_revision(mode_revision, force)
  if force && revision > mod_revision then
...
local function watch(premature, etcd_conf)
  ...
  local res, err = httpc:request({
    path = "/v3/watch",
    method = "POST",
    body = json.encode(create_request),
  })
  if not res then
  ...
  local ok, err = ngx_timer_at(1, watch, etcd_conf)
  if not ok then
      errlog("faield start watch: ", err)
  end
end
